...
test("Constructor works", () => {
  const path = new Path([
    [PathNodeType.OBJECT_KEY, "a"],
    [PathNodeType.ARRAY_INDEX, 1],
    [PathNodeType.OBJECT_KEY, "c"]
  ]);
...
test("Parse path 'a.b.c.d'", () => {
  ...
  expect(parsedPath.nodes).toEqual([
    [PathNodeType.OBJECT_KEY, "a"],
    [PathNodeType.OBJECT_KEY, "b"],
    [PathNodeType.OBJECT_KEY, "c"],
    [PathNodeType.OBJECT_KEY, "d"]
  ]);
});
...
test("Creating a path string", () => {
  const path = new Path([
    ...
    [PathNodeType.OBJECT_KEY, "c"],
    [PathNodeType.OBJECT_KEY, "d"],
    [PathNodeType.ARRAY_INDEX, 2],
    [PathNodeType.ARRAY_INDEX, 0]
  ]);
...
test("Using the parser on path nodes", () => {
  ...
  const constructedPath = new Path([
    [PathNodeType.OBJECT_KEY, "a"],
    [PathNodeType.OBJECT_KEY, "b"],
    [PathNodeType.ARRAY_INDEX, 1],
    [PathNodeType.OBJECT_KEY, "c"]
